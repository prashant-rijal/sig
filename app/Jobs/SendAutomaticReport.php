...
class SendAutomaticReport implements ShouldQueue
{
    ...
    public function getAutomaticReportPDFBySchema($projectId, $reportType, $params, $schema) {
        ...
        $pdf->reportEndPaging();
        if (!\File::exists(public_path() . '/auto-reports/')) {
            \File::makeDirectory(public_path() . '/auto-reports/', 0777, true);
        }
        $filename = 'project-' . $projectId . '-' . $reportType . '-' . 'pdf-report.pdf';
        $filePath = public_path() . '/auto-reports/' . $filename;
